...
| 📚 학습(정보) | 🤝 연결(동행) | 🗣️ 소통(커뮤니티) | 🎁 혜택(제휴) | 🛡️ 안전(운영) |
|---|---|---|---|---|
| 팀/룰/용어/관전 포인트 | 날짜/구장/팀 기반 모집·참여 | 경기 전/중/후 소통 | 주변 맛집/놀거리/굿즈 | 신고·경고·제재/인증 |
| 선생님(인증) 편집 | 매칭/확정 흐름 | 질문(Q&A) | 쿠폰/이벤트 확장 | 욕설/도배 필터+검수 |
...
### 🧑‍🏫 선생님(인증 사용자)
팀 위키 작성/수정(출처 기반) → 새싹 질문 답변 → 기여도(확장)

### 🧢 직관러(동행)
경기 조건으로 모집글 작성 → 참가자 확인 → 확정 → 후기/평가(확장)

### 🛠️ 관리자
신고 처리 → 경고/제재 → 선생님 인증/권한 관리 → 제휴 관리(확장)
